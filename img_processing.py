...
def pdfFolderToImgs(
    ...
    """
    pdfPaths = [
        file for file in os.listdir(pdfFolderPath) if file.endswith(".pdf")
    ]
    ...
    for pdf in pdfPaths:
        ...
        for i, p in enumerate(pages):
            ...
            imgPath = Path.joinpath(
                outputLoc,
                str(pdf.split(".pdf")[0]) + "pg_" + str(imgNum) + ".jpg",
            )
            p.save(imgPath)
...
def threshImg(img, minThresh=100, maxThresh=255):
    ...
    ret, imgThresh = cv2.threshold(
        imgGray, minThresh, maxThresh, cv2.THRESH_BINARY_INV
    )
...
def alignForm(img, width=0, height=0, isTemplate=False):
    ...
    if isTemplate == True:
        imgResize = cv2.resize(
            img, (0, 0), fx=xScale, fy=yScale, interpolation=cv2.INTER_AREA
        )
        print(
            "AlignImage: img resized by {}{} in width and {}{} in height".format(
